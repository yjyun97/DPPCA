...
fig = figure('Units','centimeters', ...
    'Position',[0 0 TILESIZE * 4.6  TILESIZE + LEGEND_SPACE], ...
    'PaperUnits','centimeters','PaperPositionMode','manual', ...
    'Renderer','painters');

tcl = tiledlayout(fig,1,4,'TileSpacing','compact','Padding','compact');
...
proj_titles = arrayfun(@(b,p) sprintf('$\\beta = %g$ (%g-AGDP)', b, p), ...
    beta_vals, priv_vals, 'UniformOutput', false);
x_all = []; y_all = [];
...
for j = 1:numel(proj_files)
    ...
    for i = 1:numel(vec_names)
        ...
        if j == 1
            h_pop(i) = p; % capture once for legend
        end
    end

    grid(ax,'on'); ax.Layer='bottom';
    ax.GridColor = GRIDCOLOR; ax.GridAlpha = 0.9;
    axis(ax,'equal'); pbaspect(ax,[1 1 1]);
    ax.FontSize = FONTSIZE; ax.TickLabelInterpreter='latex'; ax.Box='on';
    xlim(ax, xlim_all); ylim(ax, ylim_all);

    title(ax, proj_titles{j}, 'FontSize', FONTSIZE, 'Interpreter','latex');
...
if WHICH_DATA == "survey"
    ...
    leg = legend(h_pop(1, legend_order), vec_names_legend(legend_order), ...
    'Interpreter', 'latex', ...
    'Orientation','horizontal');
else 
    leg = legend(h_pop(1,:), vec_names_legend, 'Interpreter', 'latex', ...
        'Orientation','horizontal');
end
...
print(fig,'plots/' + WHICH_DATA + '_Figure4.pdf','-dpdf','-painters');